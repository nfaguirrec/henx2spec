...
!!                                                                                 !
!!    Authors:                                                                     !
!!    (2010-2012) Néstor F. Aguirre                                                !
!!                nfaguirrec@gmail.com                                             !
!!                nfaguirrec@iff.csic.es                                           !
!!    (    -2010) Ricardo Perez de Tudela                                          !
!!                rpt@imaff.cfmac.csic.es                                          !
!!    (    -2009) Pablo Villarreal Herrán                                          !
...
module ThrularNumerovMethod_
	...
	type, public :: ThrularNumerovMethod
		real(8) :: rMass
		integer :: nStates
		type(NFunction) :: potential
		real(8), allocatable :: eigenvalue(:)
		type(NFunction), allocatable :: eigenfunction(:)
	...
	subroutine init( this, potential, nStates, rMass )
		...
		type(NFunction), intent(in) :: potential
		...
		allocate( this.eigenvalue(potential.xGrid.size) )
	...
	subroutine run( this )
		...
		allocate( eigenfunction(this.nStates,this.potential.xGrid.size) )
		
		call thrularnumerov( this.rMass, this.potential.xGrid.min, this.potential.xGrid.max, &
							 this.potential.xGrid.size, this.potential.yArray, this.nStates, 1, &
							 this.eigenvalue, eigenfunction, nBound )
		do i=1,this.nStates
			call this.eigenfunction(i).fromArray( this.potential.xGrid, eigenfunction(i,:) )
! 			this.eigenfunction(i).yArray(:) = eigenfunction(i,:)
		end do
